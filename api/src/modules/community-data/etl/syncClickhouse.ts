...
// ingestion via `npm run etl:clickhouse` or `npm run ingest:all`.
...
async function buildFoodBankSummaryRows(): Promise<FoodBankSummaryRow[]> {
  ...
  const { rows } = await pgPool.query(
    `SELECT id, name, meals_provided, pounds_distributed, counties_served FROM feeding_america_food_banks`
  );
