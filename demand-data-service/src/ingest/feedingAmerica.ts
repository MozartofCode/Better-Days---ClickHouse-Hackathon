...
// numeric/opaque id — verified against the live API on 2026-08-28.
...
async function call<T>(endpoint: string, params: Record<string, string> = {}): Promise<T> {
  if (!env.feedingAmericaApiKey) {
    throw new Error(
      "FEEDINGAMERICA_API_KEY is not set. See demand-data-service/.env.example and the README."
    );
  }

  const url = new URL(`${env.feedingAmericaBaseUrl.replace(/\/$/, "")}/${endpoint}`);
  ...
  const doFetch = () =>
    fetch(url.toString(), {
      headers: { "X-API-Key": env.feedingAmericaApiKey },
    });
